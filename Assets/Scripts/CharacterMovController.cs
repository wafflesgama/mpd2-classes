...
public class CharacterMovController : MonoBehaviour
{
    ...
    public float jetpakcMaxYSpeed = 20;
    public float fuelSpendRate = 0.1f;
    public float fuelGainRate = 0.5f;
    ...
    void Start()
    {
        ...
        inputHandler.input_sprint.Onpressed.Subscribe(eventHandler, TurnOnJetpack);
        inputHandler.input_sprint.Onreleased.Subscribe(eventHandler, TurnOffJetpack);
    }
    ...
    private void UseJetpack()
    {
        ...
        fuel = Mathf.Clamp(fuel - fuelSpendRate, 0, maxFuel);
        ...
        if (rb.velocity.y > jetpakcMaxYSpeed) return;

        rb.AddForce(Vector3.up * jetpackForce * rb.mass);
    }

    private void GainFuel()
    {
        if (!isGroundNear || isUsingJetpack) return;

        fuel = Mathf.Clamp(fuel + fuelGainRate, 0, maxFuel);
    }
